<template>
	<Modal title="临时聊天室" v-model="chatVisible" draggable footer-hide :width="580" @on-cancel="cancel">
		<view class="chat">
			<view class="chat-message-body" id="chatform" @scroll="scroll">
				<Spin v-if="loading">
					<Icon type="ios-loading" size=18 class="spin-icon-load"></Icon>
				</Spin>
				<view dis-hover v-for="(item,index) in data" :key="index" class="message-card">
					<view :class="item.isOneself == 1?'message-row-right': 'message-row-left'">
						<img :src="item.avatar?item.avatar:defualtAvatar" height="35" width="35">
						<view class="message-content">
							<view
								:style="item.isOneself == 1?'text-align:right;display: flex;flex-direction:row-reverse':''">
								{{item.nickName}}
			...
			<Input class="chatin" v-model="form.msg" type="textarea" style="margin:10px 0 solid;"
				placeholder="是男人,就来击剑！" :rows="4" />
		</view>
