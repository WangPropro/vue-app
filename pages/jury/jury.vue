<template>
	<view>
		<!-- <view v-if="new Date().getTime()>"></view> -->
		<view class="u-flex u-row-between u-m-l-30 u-m-r-30">
			<text class="title u-font-30 ">
				<h1>陪审团</h1>
				<h3>你认为当前事件是骗局吗？请根据你的经验做出判断。</h3>截止时间：{{time}}
			</text>
			<u-image width="75%" height="300rpx" src="/static/shenpan.webp"></u-image>
		</view>
			...
			<view class="right">
				<view class="top">
					...
					<view class="like" :class="{ highlight: res.isLike }">
						...
						<u-icon v-if="!res.isLike" name="thumb-up" :size="50" color="#5677fc" @click="getLike(index)">
						</u-icon>
				...
				<view class="bottom">
					<view class="content2">陪审团留言</view>
					<u-button class="reply" @click="toSetReply()" size="mini">写留言</u-button>
				</view>
...
<script>
	import baifenbi from "@/pages/jury/baifenbi.vue";
	export default {
		data() {
			return {
				leftPercent: '',
		...
		methods: {
			...
			changey(index) {
				...
				if (this.commentList[index].status == 0) {
					uni.showToast({
						title: "评价成功，感谢您的参与！"
					})
